...
public class GriefWarnings {
    ...
    public boolean checkGriefLogic(Building build) {
    ...
    public void handleBlockBuildEndEvent(BlockBuildEndEvent event) {
        if (!event.breaking && checkGriefLogic(event.tile.build)) {
    ...
    public void stopLogic(LogicBuild logicBuild) {
        ...
        if(logicBuild.code != null) {
            StringBuffer sb = new StringBuffer();
            Pattern pattern = Pattern.compile("\njump (\\d+)");
            Matcher matcher = pattern.matcher(logicBuild.code);
            sb.append("end\n");
            while(matcher.find()) {
                int jump = Integer.parseInt(matcher.group(1));
                matcher.appendReplacement(sb, "\njump " + (jump == -1 ? jump : jump + 1 ));
            }
            matcher.appendTail(sb);
            logicBuild.configure(LogicBlock.compress(sb.toString(), logicBuild.relativeConnections()));
        }
    }
    ...
    public void handleBuildSelectEvent(BuildSelectEvent event) {
        ...
        if (!breaking && team == player.team() && builder.getPlayer() != null && builder.buildPlan() != null) {
            ...
            /*
             * else if (coreDistance < 10 && cblock instanceof ItemLiquidGenerator) { String
             * message = "[scarlet]WARNING[] " + formatPlayer(player) +
             * " is building a generator at " + formatTile(tile) + " [stat]" +
             * Math.round(coreDistance) + "[] blocks from core. [stat]" +
             * Math.round(progress * 100) + "%"; sendMessage(message, 2); }
             */
