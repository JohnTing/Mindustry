...
public class DesktopInput extends InputHandler{
    ...
    public void buildUI(Group group){
        ...
        group.fill(t -> {
          t.visible(() -> freecam != 0 && Core.settings.getBool("hints") && ui.hudfrag.shown && !(Core.settings.getBool("hints") && lastSchematic != null && !selectRequests.isEmpty()));
          t.bottom();
          t.table(Styles.black6, b -> {
              b.defaults().left();
              b.label(() -> String.format("[accent][[%s][] to switch freecam mode", Core.keybinds.get(Binding.freecam).key.toString())).style(Styles.outlineLabel);
          }).margin(6f);
        });
        ...
        group.fill(t -> {
            ...
            t.table(Styles.black6, b -> {
                ...
                b.label(() -> ((!isBuilding || !wasBuilding) && !Core.settings.getBool("buildautopause") && !player.unit().isBuilding() ?
                    ...
                    "\n" + Core.bundle.format("rebuildbuilding", Core.keybinds.get(Binding.rebuild_pain).key.toString())                
                    :
    ...
    public void drawBottom(){
        ...
        if(!isBuilding) {
            drawDestroyed();
            if(input.keyDown(Binding.rebuild_pain)) {
    ...
    void pollInput(){
        ...
        if (Core.input.keyTap(Binding.toggle_unit_draw)) {
          UnitType.hide = !UnitType.hide;
        }
        
        if(!isBuilding) {
            /*
            if(input.keyTap(Binding.rebuild_pain)) {
                for (BlockPlan block : player.team().data().blocks) {
                    if(Mathf.within(((Core.input.mouseWorldX()+4)/8), ((Core.input.mouseWorldY()+4)/8), block.x, block.y, 100f/8)) {
                        player.unit().addBuild(new BuildPlan(block.x, block.y, block.rotation, content.block(block.block), block.config));
                        
                    }
                }
            }*/
            if(input.keyDown(Binding.rebuild_pain) && Mathf.dst2(Core.input.deltaX(), Core.input.deltaY()) > 10f ) {
